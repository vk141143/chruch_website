...
import {
  ...
  FileText, BookOpen, Bookmark, Crown, ArrowLeft, LayoutGrid, Tv2,
} from "lucide-react";
...
type MainTab    = "library" | "course";
type ContentTab = "notes" | "resources" | "refs" | "bookmarks";

export function VideoLearning({ course, isPremium, onViewChange }: Props) {
  const [tab, setTab]               = useState<MainTab>(course ? "course" : "library");
  const [activeLesson, setLesson]   = useState(LESSONS[5]);
  ...
  function markDone() {
    ...
    if (idx < LESSONS.length - 1) setLesson(LESSONS[idx + 1]);
  }

  function saveNote() {
    if (!note.trim()) return;
    setSavedNotes((p) => [`[${activeLesson.title}]: ${note.trim()}`, ...p]);
    setNote("");
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex gap-1 rounded-xl bg-cream p-1 w-fit">
          <button
            ...
            className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${
              tab === "library" ? "bg-background text-navy shadow-sm" : "text-muted-foreground hover:text-navy"
            }`}
          >
            <LayoutGrid className="h-4 w-4" /> Video Library
          </button>
          {course && (
            <button
              onClick={() => setTab("course")}
              className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${
                tab === "course" ? "bg-background text-navy shadow-sm" : "text-muted-foreground hover:text-navy"
              }`}
            >
              <Tv2 className="h-4 w-4" /> Course Player
            </button>
          )}
        </div>
      </div>
      ...
      {tab === "library" && (
        <div className="flex-1 overflow-y-auto">
          <VideoGrid isPremium={isPremium} onViewChange={onViewChange} />
        </div>
      )}
                ...
                {isLocked ? (
                  <div className="flex flex-col items-center gap-4 text-white text-center px-6">
                    ...
                    <div>
                      ...
                      <p className="text-white/60 text-sm mt-1">Upgrade to unlock this lesson and all premium content</p>
                    </div>
                    <button
                      onClick={() => onViewChange("subscription")}
                      className="flex items-center gap-2 rounded-full bg-gradient-gold px-6 py-3 text-sm font-semibold text-navy shadow-gold hover:scale-[1.02] transition-transform"
                    >
                  ...
                  {(["notes", "resources", "refs", "bookmarks"] as const).map((t) => (
                    <button
                      ...
                      className={`flex-1 rounded-lg py-2 text-xs font-medium capitalize transition-all ${
                        contentTab === t ? "bg-background text-navy shadow-sm" : "text-muted-foreground hover:text-navy"
                      }`}
                    >
                      {t === "refs" ? "Bible Refs" : t}
                    </button>
                ...
                <div className="mt-4">
                  {contentTab === "notes" && (
                    <div className="space-y-3">
                      ...
                      {savedNotes.map((n, i) => (
                        <div key={i} className="flex gap-2 rounded-xl bg-cream p-3 text-sm text-navy">
                          <FileText className="h-4 w-4 text-gold shrink-0 mt-0.5" /> {n}
                        </div>
                      ))}
                      {savedNotes.length === 0 && <p className="text-sm text-muted-foreground">No notes yet.</p>}
                    </div>
                  )}
              ...
              <div className="divide-y divide-border">
                {LESSONS.map((lesson, i) => {
                  ...
                  return (
                    <button
                      key={lesson.id}
                      onClick={() => !locked && setLesson(lesson)}
                      disabled={locked}
